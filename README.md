...
- 🤖 **AI 生成日记**：多模态 LLM 看图 + 上下文（时间/地点/备注）→ 日记；可编辑 / 重新生成 / 换文风；幻觉约束（只写照片可见事实）；模型不支持图片时自动降级纯文本模式
- 🎨 **文风系统**：内置 温暖/文艺/浪漫/简洁/痛苦 五种，支持自定义提示词命名保存，生成时下拉选择
- 🎨 **多主题**：6 套主题（浪漫粉/落日橙/清新绿/静谧蓝/星空夜/暖棕），一键切换持久化
- ⏱ **四页导航**：主页（在一起天数/下一个纪念日/统计）/ 时间线（节日·纪念日标记）/ 地点线（按地点聚合回忆）/ 设置
